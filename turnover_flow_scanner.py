...
PREMIUM_BASED_OPTION_ACTIONS = {"CALL_BUY", "PUT_BUY", "CALL_UNW", "PUT_UNW"}
FIXED_BASED_OPTION_ACTIONS = {"CALL_WRITER", "PUT_WRITER", "CALL_SC", "PUT_SC"}
FIXED_OPTION_TURNOVER_PER_UNIT = 125000
...
def calculate_option_turnover(action_type, lots, price, lot_size):
    if action_type in PREMIUM_BASED_OPTION_ACTIONS:
        return (lots * price * lot_size) if price else 0
    if action_type in FIXED_BASED_OPTION_ACTIONS:
        return lots * FIXED_OPTION_TURNOVER_PER_UNIT * lot_size
    return 0
...
async def process_summary(context: ContextTypes.DEFAULT_TYPE):
    ...
    # Safety: Swap buffer to avoid missing messages during processing
    ...
    for alert in batch:
        ...
        if zone: # Logic for Options
            opt_data[sym][act][zone] += lots
            opt_turn[sym][act][zone] += calculate_option_turnover(act, lots, price, lot_size)
        else: # Logic for Futures
